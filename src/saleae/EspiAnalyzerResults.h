...
class EspiAnalyzerResults : public AnalyzerResults
{
  public:
    ...
    //
    // `text` is the field's own value, `compact` is what its children resolved
    // to, and `detail` is those children in full. They are kept apart rather
    // than concatenated because a bubble has to be readable at several widths:
    // see GenerateBubbleText.
